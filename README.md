...
This will create SmartListing named `:users` consisting of ActiveRecord scope `User.active` elements and rendered by partial `users/listing`. You can also use arrays instead of ActiveRecord collections. Just put `array: true` option.
...
* `Builder#pagination_per_page_links` - display some link that allow you to customize Pagy's items per page,
...
SmartListing Pagy for pagination https://github.com/ddnexus/pagy
